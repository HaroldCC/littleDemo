...
> Created Time    : 2020年02月25日  13时57分59秒
************************************************************************/
/***** 单链表实现(使用友元类) *****/
...
template <class T>
class LinkedList; // 友元类前置声明

// 节点结构
template <typename T>
class Node
{
    friend class LinkedList<T>; // 声明友元类
private:
    T element;
    Node<T> *next;
...
public:
    ...
    // 根据索引设置索引值
    T setVal(const int index, T &element);
    // 根据索引删除元素并返回删除的元素
...
// 设置指定索引处的值,将原来值返回
template <typename T>
T LinkedList<T>::setVal(const int index, T &element)
{
    Node<T> *nodePtr = head;
...
template <typename T>
void LinkedList<T>::printArray() const
{
